...
class DBUtil:
	def __init__(self):
		self.len_of_mac = 12
		self.operation = {'insert' : 0, 'delete' : 1}
	...
	def insert_colons_in_string(self, inp_str):
		return (':'.join(inp_str[i] + inp_str[i+1] for i in xrange(0, len(inp_str), 2)))
		...
		if obj is not None:
			...
			if 'hosts' in obj:
				...
				for host in hosts:
					if 'mac' in host:
						mac_addr = host['mac']
						list_of_host_macs[self.convert_mac_format(mac_addr)] = host
