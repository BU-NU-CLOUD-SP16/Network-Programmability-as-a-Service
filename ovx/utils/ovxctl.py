...
def do_createNetwork(gopts, opts, args):
    if len(args) != 4:
        print "createNetwork : Must specify controllerUrls, network_ip, network_mask tenantId"
        sys.exit()
    req = { "controllerUrls" : buildControllerList(args[0]), \
                 "networkAddress" : args[1], "mask" : int(args[2]), "tenantId" : int(args[3])  }
...
DESCS = {
    ...
    'createNetwork' : ("Creates a virtual network",
                       ("Creates a virtual network. Input: protocol, controllerIP, controller port, ip address, mask, tenantId. "
                        "\nExample: createNetwork tcp 1.1.1.1 6634 192.168.1.0 24 10")),
